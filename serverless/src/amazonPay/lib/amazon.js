...
function parseMwsResponse(method, headers, response, callback) {
  ...
  if (headers && headers['content-type'] == 'text/xml') {
    xml2js.parseString(response, { explicitArray: false }, function(
      err,
      result
    ) {
      if (err) {
...
function parseSNSResponse(response, callback) {
    ...
    if (!response.hasOwnProperty(required[i])) {
      return callback(
        error.missingParameter(
          'Missing parameter on SNS response: ' + required[i]
        )
      )
    }
  }
  ...
  if (response.SignatureVersion != 1) {
    return callback(
      error.invalidSignatureVersion(
        'Unknown SNS Signature version: ' + response.SignatureVersion
      )
    )
  }
  ...
  if (
    ...
    return callback(
      error.invalidCertificateDomain(
  ...
  request(response.SigningCertURL, function(err, res, cert) {
    ...
    if (!isValid) {
      return callback(
        error.signatureMismatch('Signature mismatch, unverified response')
      )
    }
